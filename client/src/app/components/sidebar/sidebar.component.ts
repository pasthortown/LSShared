...
export class SidebarComponent implements OnInit {
  ...
  checkRols() {
    const rols = JSON.parse(sessionStorage.getItem('rols')) as AccountRol[];
    rols.forEach(rol => {
      if (rol.administrative_rol_id === 1) {
        this.isAdmin = true;
      }
      if (rol.administrative_rol_id === 2) {
        this.isInstitute = true;
      }
      if (rol.administrative_rol_id === 3) {
