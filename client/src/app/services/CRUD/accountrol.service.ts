...
import { AccountRol } from './../../models/AccountRol';

@Injectable({
   providedIn: 'root'
})
export class AccountRolService {

   url = environment.api + 'accountrol/';
   ...
   post(accountrol: AccountRol): Promise<any> {
      return this.http.post(this.url, JSON.stringify(accountrol), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(accountrol: AccountRol): Promise<any> {
      return this.http.put(this.url, JSON.stringify(accountrol), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
