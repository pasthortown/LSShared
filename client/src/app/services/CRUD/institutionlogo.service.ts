...
export class InstitutionLogoService {
   ...
   url = environment.api + 'institutionlogo/';
   ...
   post(institutionlogo: InstitutionLogo): Promise<any> {
      return this.http.post(this.url, JSON.stringify(institutionlogo), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(institutionlogo: InstitutionLogo): Promise<any> {
      return this.http.put(this.url, JSON.stringify(institutionlogo), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
