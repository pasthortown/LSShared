...
export class PublicationTypeService {
   ...
   url = environment.api + 'publicationtype/';
   ...
   post(publicationtype: PublicationType): Promise<any> {
      return this.http.post(this.url, JSON.stringify(publicationtype), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(publicationtype: PublicationType): Promise<any> {
      return this.http.put(this.url, JSON.stringify(publicationtype), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
