...
export class InstitutionInternalRolService {

   url = environment.api + 'institutioninternalrol/';
   ...
   post(institutioninternalrol: InstitutionInternalRol): Promise<any> {
      return this.http.post(this.url, JSON.stringify(institutioninternalrol), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(institutioninternalrol: InstitutionInternalRol): Promise<any> {
      return this.http.put(this.url, JSON.stringify(institutioninternalrol), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
