...
export class AuthorService {
   ...
   post(author: Author): Promise<any> {
      return this.http.post(this.url, JSON.stringify(author), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(author: Author): Promise<any> {
      return this.http.put(this.url, JSON.stringify(author), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
