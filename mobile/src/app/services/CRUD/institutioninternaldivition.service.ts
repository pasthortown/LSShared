...
export class InstitutionInternalDivitionService {

   url = environment.api + 'institutioninternaldivition/';
   ...
   post(institutioninternaldivition: InstitutionInternalDivition): Promise<any> {
      return this.http.post(this.url, JSON.stringify(institutioninternaldivition), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(institutioninternaldivition: InstitutionInternalDivition): Promise<any> {
      return this.http.put(this.url, JSON.stringify(institutioninternaldivition), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
