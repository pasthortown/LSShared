...
export class AdministrativeRolService {

   url = environment.api + 'administrativerol/';
   ...
   post(administrativerol: AdministrativeRol): Promise<any> {
      return this.http.post(this.url, JSON.stringify(administrativerol), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(administrativerol: AdministrativeRol): Promise<any> {
      return this.http.put(this.url, JSON.stringify(administrativerol), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
