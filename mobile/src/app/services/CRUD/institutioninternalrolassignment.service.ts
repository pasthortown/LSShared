...
export class InstitutionInternalRolAssignmentService {

   url = environment.api + 'institutioninternalrolassignment/';
   ...
   post(institutioninternalrolassignment: InstitutionInternalRolAssignment): Promise<any> {
      return this.http.post(this.url, JSON.stringify(institutioninternalrolassignment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(institutioninternalrolassignment: InstitutionInternalRolAssignment): Promise<any> {
      return this.http.put(this.url, JSON.stringify(institutioninternalrolassignment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
