...
export class PersonService {
   ...
   post(person: Person): Promise<any> {
      return this.http.post(this.url, JSON.stringify(person), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(person: Person): Promise<any> {
      return this.http.put(this.url, JSON.stringify(person), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
