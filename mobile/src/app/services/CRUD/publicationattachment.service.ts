...
export class PublicationAttachmentService {

   url = environment.api + 'publicationattachment/';
   ...
   post(publicationattachment: PublicationAttachment): Promise<any> {
      return this.http.post(this.url, JSON.stringify(publicationattachment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(publicationattachment: PublicationAttachment): Promise<any> {
      return this.http.put(this.url, JSON.stringify(publicationattachment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
