...
export class InstitutionService {
   ...
   url = environment.api + 'institution/';
   ...
   post(institution: Institution): Promise<any> {
      return this.http.post(this.url, JSON.stringify(institution), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(institution: Institution): Promise<any> {
      return this.http.put(this.url, JSON.stringify(institution), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
