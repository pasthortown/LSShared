...
export class PublicationCommentService {
   ...
   url = environment.api + 'publicationcomment/';
   ...
   post(publicationcomment: PublicationComment): Promise<any> {
      return this.http.post(this.url, JSON.stringify(publicationcomment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(publicationcomment: PublicationComment): Promise<any> {
      return this.http.put(this.url, JSON.stringify(publicationcomment), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
